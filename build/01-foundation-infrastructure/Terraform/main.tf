...
resource "aws_security_group" "alb" {
  ...
  ingress {
    description = "Allow HTTP from the internet for demo application entry"
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
  ...
  ingress {
    description = "Allow HTTPS from the internet for secure application entry"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_security_group" "database" {
  ...
  ingress {
    description     = "Allow SQL Server access from application tier only"
    from_port       = 1433
    to_port         = 1433
    protocol        = "tcp"
